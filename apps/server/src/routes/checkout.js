...
export async function checkoutRoutes({ recipient, network, mintAddress }) {
  ...
  router.post('/checkout', async (req, res) => {
    try {
      ...
      // 🪙 Add SPL-token param when paying in USDC
      if (asset === 'SPL' || asset === 'USDC') {
        if (!mintAddress) {
          return res.status(400).json({ ok: false, error: 'MINT_ADDRESS missing in env' });
        }
